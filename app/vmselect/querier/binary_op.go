...
func adjustBinaryOpTags(be *logql.BinaryOpExpr, left, right []*timeseries) ([]*timeseries, []*timeseries, []*timeseries, error) {
...
func ensureSingleTimeseries(side string, be *logql.BinaryOpExpr, tss []*timeseries) error {
	if len(tss) == 0 {
...
func groupJoin(singleTimeseriesSide string, be *logql.BinaryOpExpr, rvsLeft, rvsRight, tssLeft, tssRight []*timeseries) ([]*timeseries, []*timeseries, error) {
...
func resetMetricGroupIfRequired(be *logql.BinaryOpExpr, ts *timeseries) {
	if logql.IsBinaryOpCmp(be.Op) && !be.Bool {
...
func createTimeseriesMapByTagSet(be *logql.BinaryOpExpr, left, right []*timeseries) (map[string][]*timeseries, map[string][]*timeseries) {
	groupTags := be.GroupModifier.Args
	groupOp := strings.ToLower(be.GroupModifier.Op)
	if len(groupOp) == 0 {
		groupOp = "ignoring"
