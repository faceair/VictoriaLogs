...
func getRollupAggrFuncNames(expr logql.Expr) ([]string, error) {
	afe, ok := expr.(*logql.AggrFuncExpr)
	if ok {
	...
	if se, ok := arg.(*logql.StringExpr); ok {
		aggrFuncNames = append(aggrFuncNames, se.S)
	} else {
		fe, ok := arg.(*logql.FuncExpr)
		if !ok || fe.Name != "" {
...
func getRollupConfigs(name string, rf rollupFunc, expr logql.Expr, start, end, step, window int64, lookbackDelta int64, sharedTimestamps []int64) (
