...
func adjustCmpOps(e logql.Expr) logql.Expr {
	logql.VisitAll(e, func(expr logql.Expr) {
		be, ok := expr.(*logql.BinaryOpExpr)
		if !ok {
			return
		}
		if !logql.IsBinaryOpCmp(be.Op) {
			return
		}
		if _, ok := be.Left.(*logql.NumberExpr); !ok {
			return
		}
...
func parsePromQLWithCache(q string) (logql.Expr, error) {
