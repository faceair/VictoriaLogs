...
func evalExpr(ec *EvalConfig, e logql.Expr, isRoot bool) ([]*timeseries, error) {
	...
	if ae, ok := e.(*logql.AggrFuncExpr); ok {
	...
	if ne, ok := e.(*logql.NumberExpr); ok {
		rv := evalNumber(ec, ne.N)
		return rv, nil
	}
	if se, ok := e.(*logql.StringExpr); ok {
...
func tryGetArgRollupFuncWithMetricExpr(ae *logql.AggrFuncExpr) (*logql.FuncExpr, newRollupFunc) {
	if len(ae.Args) != 1 {
		return nil, nil
	}
	e := ae.Args[0]
	...
	if re, ok := e.(*logql.RollupExpr); ok {
		if me, ok := re.Expr.(*logql.MetricExpr); !ok || me.IsEmpty() || re.ForSubquery() {
			return nil, nil
		}
		// e = metricExpr[d]
		fe := &logql.FuncExpr{
	...
	if me, ok := arg.(*logql.MetricExpr); ok {
		if me.IsEmpty() {
			return nil, nil
		}
		// e = rollupFunc(metricExpr)
		return &logql.FuncExpr{
			Name: fe.Name,
			Args: []logql.Expr{me},
		}, nrf
	}
	if re, ok := arg.(*logql.RollupExpr); ok {
		if me, ok := re.Expr.(*logql.MetricExpr); !ok || me.IsEmpty() || re.ForSubquery() {
			return nil, nil
		}
...
func evalRollupFuncArgs(ec *EvalConfig, fe *logql.FuncExpr) ([]interface{}, *logql.RollupExpr, error) {
	var re *logql.RollupExpr
	rollupArgIdx := getRollupArgIdx(fe.Name)
...
func getRollupExprArg(arg logql.Expr) *logql.RollupExpr {
	re, ok := arg.(*logql.RollupExpr)
	if !ok {
		// Wrap non-rollup arg into metricsql.RollupExpr.
		return &logql.RollupExpr{
			Expr: arg,
		}
	}
	if !re.ForSubquery() {
	...
	me, ok := re.Expr.(*logql.MetricExpr)
	if !ok {
	...
	reNew := *re
	reNew.Expr = &logql.FuncExpr{
		Name: "default_rollup",
		Args: []logql.Expr{
			&logql.RollupExpr{Expr: me},
		},
	}
	return &reNew
}

func evalRollupFunc(ec *EvalConfig, name string, rf rollupFunc, expr logql.Expr, re *logql.RollupExpr, iafc *incrementalAggrFuncContext) ([]*timeseries, error) {
	ecNew := ec
	var offset int64
	if len(re.Offset) > 0 {
		var err error
		offset, err = logql.DurationValue(re.Offset, ec.Step)
		if err != nil {
			return nil, err
		}
	...
	var rvs []*timeseries
	var err error
	if me, ok := re.Expr.(*logql.MetricExpr); ok {
...
func evalRollupFuncWithSubquery(ec *EvalConfig, name string, rf rollupFunc, expr logql.Expr, re *logql.RollupExpr) ([]*timeseries, error) {
	// TODO: determine whether to use rollupResultCacheV here.
	var step int64
	if len(re.Step) > 0 {
		var err error
		step, err = logql.PositiveDurationValue(re.Step, ec.Step)
		if err != nil {
			return nil, err
		}
	} else {
	...
	if len(re.Window) > 0 {
		var err error
		window, err = logql.PositiveDurationValue(re.Window, ec.Step)
		if err != nil {
			return nil, err
		}
	}
...
func evalRollupFuncWithMetricExpr(ec *EvalConfig, name string, rf rollupFunc,
	...
	if len(windowStr) > 0 {
		var err error
		window, err = logql.PositiveDurationValue(windowStr, ec.Step)
		if err != nil {
			return nil, err
		}
	}
	...
	rss, isPartial, err := netstorage.ProcessSearchQuery(ec.AuthToken, sq, 1, ec.Deadline)
	if err != nil {
		return nil, err
	}
	if isPartial && ec.DenyPartialResponse {
