...
func doTransformValues(arg []*timeseries, tf func(values []float64), fe *logql.FuncExpr) ([]*timeseries, error) {
	for _, ts := range arg {
...
func getAbsentTimeseries(ec *EvalConfig, arg logql.Expr) []*timeseries {
	// Copy tags from arg
	rvs := evalNumber(ec, 1)
	rv := rvs[0]
	me, ok := arg.(*logql.MetricExpr)
	if !ok {
		return rvs
	}
...
func transformLabelTransform(tfa *transformFuncArg) ([]*timeseries, error) {
	...
	r, err := logql.CompileRegexp(regex)
	if err != nil {
...
func transformLabelReplace(tfa *transformFuncArg) ([]*timeseries, error) {
	...
	r, err := logql.CompileRegexpAnchored(regex)
	if err != nil {
		return nil, fmt.Errorf(`cannot compile regex %q: %w`, regex, err)
	}
...
func transformLabelMatch(tfa *transformFuncArg) ([]*timeseries, error) {
	...
	r, err := logql.CompileRegexpAnchored(labelRe)
	if err != nil {
...
func transformScalar(tfa *transformFuncArg) ([]*timeseries, error) {
	...
	if se, ok := tfa.fe.Args[0].(*logql.StringExpr); ok {
