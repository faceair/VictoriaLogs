...
var (
	readCalls  = metrics.NewCounter(`vm_protoparser_read_calls_total{type="importer"}`)
	readErrors = metrics.NewCounter(`vm_protoparser_read_errors_total{type="importer"}`)
	rowsRead   = metrics.NewCounter(`vm_protoparser_rows_read_total{type="importer"}`)
)
